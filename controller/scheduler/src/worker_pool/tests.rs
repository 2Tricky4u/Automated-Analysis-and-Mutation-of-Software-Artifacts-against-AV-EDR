...
async fn test_worker_assignment() {
    ...
    // Assign job
    let address = pool.assign_worker("worker-01", "job-000001").await.unwrap();
    ...
    assert!(pool.assign_worker("worker-01", "job-000002").await.is_err());

    // Release worker
    pool.release_worker("worker-01").await.unwrap();
...
async fn test_available_workers() {
    ...
    pool.mark_connected("worker-01").await.unwrap();
    pool.mark_connected("worker-02").await.unwrap();
    // worker-03 is disabled, so doesn't matter if connected

    let available = pool.get_available_workers().await;
    assert_eq!(available.len(), 2);
    assert!(available.contains(&"worker-01".to_string()));
    assert!(available.contains(&"worker-02".to_string()));

    // Assign one worker
    pool.assign_worker("worker-01", "job-000001").await.unwrap();
...
async fn test_health_check() {
    ...
    // Initially available
    assert_eq!(pool.count_by_status(WorkerStatus::Available).await, 1);

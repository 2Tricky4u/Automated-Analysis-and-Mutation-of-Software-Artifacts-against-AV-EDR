...
use edr::{
    ...
    JobRequest, JobResponse, JobStatusRequest, JobStatusResponse,
...
impl Controller for SchedulerService {
    ...
    async fn submit_triage(
        ...
        let req = request.into_inner();
        let triage_id = format!("triage-{}", std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs());
        
        info!("Triage submitted for job {}: detected={}", req.job_id, req.detected);

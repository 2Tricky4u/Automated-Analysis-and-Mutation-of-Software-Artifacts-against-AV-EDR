    ...
    fn new() -> Self {
        ...
        let mutations = vec![
            MutationInfo {
                ...
                params: [("delay_load".to_string(), "true".to_string())].iter().cloned().collect(),
                success_rate: 0.5,
                total_attempts: 0,
            },
            MutationInfo {
                ...
                category: "ast".to_string(),
                params: [("complexity".to_string(), "2".to_string())].iter().cloned().collect(),
                success_rate: 0.5,
                total_attempts: 0,
            },
            MutationInfo {
            ...
            MutationInfo {
                ...
                params: [("strategy".to_string(), "random".to_string())].iter().cloned().collect(),
                success_rate: 0.5,
                total_attempts: 0,
            },
            MutationInfo {
                ...
                category: "source".to_string(),
                params: [("depth".to_string(), "1".to_string())].iter().cloned().collect(),
                success_rate: 0.5,
                total_attempts: 0,
            },
        ];
...
impl Selector for SelectorService {
    async fn select_mutation(
        ...
        info!("Selecting mutations for job: {}, round: {}", job_id, round_id);

        // Extract avoid features from previous feedback
        let avoid_features = req.previous_feedback
            .as_ref()
            .map(|f| f.avoid_features.clone())
            .unwrap_or_default();

        let evasion_score = req.previous_feedback
            .as_ref()
        ...
        let mutations: Vec<Mutation> = selected.iter()
            .map(|m| Mutation {
                id: m.id.clone(),
        ...
        let rationale = if selected.iter().any(|m| m.success_rate > 0.7) {
            format!("Exploitation: selected {} high-success mutations", selected.len())
        } else {
            format!("Exploration: trying {} new mutation strategies", selected.len())
        };

        info!("Selected mutations: {:?}", selected.iter().map(|m| &m.id).collect::<Vec<_>>());
    ...
    async fn report_outcome(
        ...
        if let Some(result) = req.result {
            ...
            let mutations: Vec<String> = result.mutations.iter()
                .map(|m| m.id.clone())
                .collect();

            debug!("Run outcome: detected={}, mutations={:?}", detected, mutations);
            ...
            for mutation in &mutations {
                if let Some(mut_info) = pool.mutations.iter_mut().find(|m| &m.id == mutation) {
                    ...
                    mut_info.success_rate = (
                        mut_info.success_rate * (mut_info.total_attempts - 1) as f64 +
                        if success { 1.0 } else { 0.0 }
                    ) / mut_info.total_attempts as f64;

                    debug!("Updated {}: success_rate={:.2}, attempts={}",

...
use crate::constants::MAX_SERIALIZED_PAYLOAD;
...
pub async fn collect_bb_coverage(
    ...
    metadata_path: &Path,
    job_id: &str,
) -> Result<TelemetryData, Box<dyn std::error::Error + Send + Sync>> {
    use tokio::fs;
...
pub async fn collect_api_checkpoints(
    ...
    job_id: &str,
) -> Result<Vec<TelemetryData>, Box<dyn std::error::Error + Send + Sync>> {
    use tokio::fs;
            ...
            Ok(checkpoint_json) => {
                ...
                let (event_type, mut metadata) = match type_field {
                    "success" => {
                        ...
                        ("artifact_success".to_string(), HashMap::new())
                    }
                    "failure" => {
                    ...
                    _ => ("checkpoint".to_string(), HashMap::new()),
                };
